...
void open_log(void) {
	...
	sprintf(log_file_name, "/home/pi/Apps/datalog/pktlog_%s_%s.json", lgwm_str, iso_date);
...
int main(int argc, char **argv)
{
	...
	while ((quit_sig != 1) && (exit_sig != 1)) {
		...
		for (i=0; i < nb_pkt; ++i) {
			...
			/* Filter packet */
			switch(p->status) {
				case STAT_CRC_OK:	break;
				case STAT_CRC_BAD:	continue; break; //Skip this packet
				case STAT_NO_CRC:	continue; break; //Skip this packet
				case STAT_UNDEFINED:continue; break; //Skip this packet
				default: continue; //Skip this packet
			}


            /* packet number */
			json_object_set_number( root_object, "Packet_No", (double) pkt_in_log+1 );
			MSG("Packet No %d\n",pkt_in_log+1);
			
			/* writing gateway ID */
            sprintf( buffer, "%08X%08X", (uint32_t)(lgwm >> 32), (uint32_t)(lgwm & 0xFFFFFFFF));
            json_object_set_string(root_object, "gateway_ID", buffer);
			MSG("Gateway ID %s\n", buffer);

			/* writing node MAC address */
			json_object_set_string(root_object, "node ID", "NULL"); // TODO: need to parse payload
			MSG("Node ID NULL\n", buffer);

			/* writing UTC timestamp*/
			sprintf( buffer, "%s", fetch_timestamp );
			json_object_set_string(root_object, "UTC Time", buffer );	// TODO: replace with GPS time when available
			MSG("UTC Time %s\n", buffer);

			/* writing RX frequency */
			json_object_set_number(root_object, "RX Freq", (double) p->freq_hz );
			
            /* writing RF chain */
			json_object_set_number(root_object, "RF chain", (double) p->rf_chain );

			/* writing RX modem/IF chain */
			json_object_set_number(root_object, "IF chain", (double) p->if_chain );
			
			/* writing status */
			switch(p->status) {
				case STAT_CRC_OK:	sprintf( buffer, "CRC_OK"); break;
				case STAT_CRC_BAD:	sprintf( buffer, "CRC_BAD"); break;
				case STAT_NO_CRC:	sprintf( buffer, "NO_CRC"); break;
				case STAT_UNDEFINED:sprintf( buffer, "UNDEF"); break;
				default: sprintf( buffer, "ERR");
			}
			json_object_set_string(root_object, "status", buffer );
			MSG("STATUS %s\n", buffer);

			/* writing payload size */
			json_object_set_number(root_object, "size", (double) p->size );
		        
			/* writing modulation */
			switch(p->modulation) {
				case MOD_LORA:	sprintf( buffer, "LORA"); break;
				case MOD_FSK:	sprintf( buffer, "FSK"); break;
				default: sprintf( buffer, "ERR");
			}
			json_object_set_string(root_object, "modulation", buffer );

			/* writing bandwidth */
			switch(p->bandwidth) {
				case BW_500KHZ:	tmp = 500000; break;
				case BW_250KHZ:	tmp = 250000; break;
				case BW_125KHZ:	tmp = 125000; break;
				case BW_62K5HZ:	tmp = 62500; break;
				case BW_31K2HZ:	tmp = 31200; break;
				case BW_15K6HZ:	tmp = 15600; break;
				case BW_7K8HZ:	tmp = 7800; break;
				case BW_UNDEFINED: tmp = 0; break;
				default: tmp = -1; 
			}
			json_object_set_number(root_object, "bandwidth", (double) tmp );
			
			/* writing datarate */
			if (p->modulation == MOD_LORA) {
				switch (p->datarate) {
					case DR_LORA_SF7:	sprintf( buffer, "SF7"); break;
					case DR_LORA_SF8:	sprintf( buffer, "SF8"); break;
					case DR_LORA_SF9:	sprintf( buffer, "SF9"); break;
					case DR_LORA_SF10:	sprintf( buffer, "SF10");break;
					case DR_LORA_SF11:	sprintf( buffer, "SF11"); break;
					case DR_LORA_SF12:	sprintf( buffer, "SF12"); break;
					default: sprintf( buffer, "ERR");
				}
			} else if (p->modulation == MOD_FSK) {
				sprintf( buffer, "%u", p->datarate );
			} else {
				sprintf( buffer, "ERR");
			}
			json_object_set_string(root_object, "datarate", buffer );

			/* writing coderate */
			switch (p->coderate) {
				case CR_LORA_4_5:	sprintf( buffer, "4/5"); break;
				case CR_LORA_4_6:	sprintf( buffer, "2/3"); break;
				case CR_LORA_4_7:	sprintf( buffer, "4/7"); break;
				case CR_LORA_4_8:	sprintf( buffer, "1/2"); break;
				case CR_UNDEFINED:	sprintf( buffer, "UNDEF"); break;
				default: sprintf( buffer, "ERR");
			}
			json_object_set_string(root_object, "coderate", buffer );
			MSG("Coderate %s\n",buffer);

			/* writing packet RSSI */
			json_object_set_number(root_object, "RSSI", (double) p->rssi  );
			
			/* writing packet average SNR */
			json_object_set_number(root_object, "SNR", (double) p->snr  );
		

			/* writing hex-encoded payload  */
			buffer1[0]= '\0';
			for (j = 0; j < p->size; ++j) {
				sprintf( buffer, "%02X", p->payload[j]);
				strcat(buffer1, buffer);
			}
			json_object_set_string(root_object, "payload", buffer1 );

            //write to JSON file
			serialized_string = json_serialize_to_string_pretty(root_value);
			fputs(serialized_string, log_file );
			MSG("Logged...OK\n");
		...
		if (time_check >= 8) {
			...
			if (difftime(now_time, log_start_time) > log_rotate_interval) {
				fprintf(log_file, "{}]\n");  //Add empty object and Close JSON object array
				fclose(log_file);
